...
def main() -> None:
    ...
    shared.add_argument('--fill',
                        action='store_true',
                        help="Perform an interior fill instead of an outline")
    shared.add_argument(
        '--stepover',
        type=float,
        default=0.4,
        help="Fill line spacing as a percentage of tool diameter")
    ...
    laser.add_argument('--kerf',
                       type=float,
                       default=0.1,
                       help="Laser beam width for compensation (mm)")
    ...
    laser.add_argument('--focus-z',
                       type=float,
                       default=0.0,
                       help="Z height for ideal laser focus")
    ...
    pen.add_argument('--pen-down-z',
                     type=float,
                     default=-1.0,
                     help="Z height to press pen to paper")
    ...
    if args.mode == 'mill':
        ...
        config = JobConfig(
            ...
            feed_ramp=args.
            feed_xy,  # Lasers do not plunge, they drop at normal feed
            feed_xy=args.feed_xy,
            compensation=args.compensation)
    ...
    logger.info(
        f"Successfully processed profile. Saved G-code to: {args.output}")
