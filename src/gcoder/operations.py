...
class SVGProfileCutter:
    ...
    def __init__(self, writer, svg_path_file, compensation, tool_dia, depth, step_down, feed_xy, feed_ramp=None):
        self.writer = writer
        self.svg_path_file = svg_path_file
        self.compensation = compensation
        self.tool_dia = tool_dia
        self.offset_distance = tool_dia / 2.0
        self.depth = depth
    ...
    def _calculate_svg_center(self, doc):
    ...
    def _extract_and_flip_points(self, path, steps=50):
    ...
    def _apply_tool_compensation(self, points, is_closed):
        ...
        if is_closed and self.compensation in ('outside', 'inside'):
            ...
            if self.compensation == 'outside':
                return geom.buffer(self.offset_distance, join_style=1)  # 1 = Round join
            else:
                return geom.buffer(-self.offset_distance, join_style=1)
    ...
    def _write_ramped_profile(self, tpath, is_closed):
